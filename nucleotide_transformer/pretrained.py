...
def load_ckpt_and_hyperparams(model_name: str, path: str) -> Tuple[hk.Params, Dict[str, Any]]:
    """
    ...
    hyperparams = json.load(
        open(path+"/"+str(MODEL_TO_REPO_MAPPING[model_name])+"/jax_model/hyperparams.json"
        )
    )

    # Download parameters
    print("loading model's weights...")
    params = joblib.load(path+"/"+str(MODEL_TO_REPO_MAPPING[model_name])+"/jax_model/pytree_ckpt.joblib"
    )
...
def load_pretrained_model(
    ...
    parameters, hyperparams = load_ckpt_and_hyperparams(model_name, model_path)
