...
def get_ckpt_and_hyperparams(model_name: str, path: str = None) -> Tuple[hk.Params, Dict[str, Any]]:
    """
    Get checkpoint and hyperparams either by downloading or loading from a path.

    Args:
        model_name: Name of the model.
        path: Path to load the model from. If None, download the model.
    ...
    if path:
        print("Loading model's hyperparameters json file... from", path)
        hyperparams = json.load(
            open(os.path.join(path, MODEL_TO_REPO_MAPPING[model_name], "jax_model", "hyperparams.json"))
        )
        print("Loading model's weights...")
        params = joblib.load(
            os.path.join(path, MODEL_TO_REPO_MAPPING[model_name], "jax_model", "pytree_ckpt.joblib")
        )
    else:
    ...
    print("Model's weights and hyperparameters loaded.")
...
def get_pretrained_model(
    ...
    parameters, hyperparams = get_ckpt_and_hyperparams(model_name, model_path)
